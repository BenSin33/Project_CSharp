  ...
  ({ label, leftIcon, error, hint, showPasswordToggle, type = "text", className = "", style, ...rest }, ref) => {
    ...
    return (
      <div className={`flex flex-col gap-1.5 ${className}`} style={style}>
        {label && (
          <label style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "14px", fontWeight: 600, color: hasError ? "#dc2626" : "#111827" }}>
            {label}
          </label>
        )}
        ...
        >
          {leftIcon && (
            <span style={{ color: "#9ca3af", flexShrink: 0, display: "flex" }}>{leftIcon}</span>
          )}
          ...
          <input
            ...
            type={inputType}
            {...rest}
            style={{ flex: 1, background: "transparent", border: "none", outline: "none", fontFamily: "'DM Sans', sans-serif", fontSize: "14px", color: "#111827", padding: "13px 0" }}
            onFocus={(e) => {
        ...
        {(error || hint) && (
          <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "12px", color: error ? "#dc2626" : "#6b7280" }}>
            {error ?? hint}
          </span>
        )}
      </div>
