...
type AdminTab = "dashboard" | "reports" | "posts" | "users"
...
export default function AdminPage() {
  ...
  const tabs: Array<{ id: AdminTab; label: string; icon: ReactNode }> = [
    ...
    { id: "reports", label: "Reports", icon: <FileWarning size={14} /> },
    { id: "posts", label: "Posts", icon: <Activity size={14} /> },
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      {activeTab === "reports" ? (
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <SectionTitle title="Quản lý reports" subtitle="Xử lý báo cáo theo trạng thái ngay trong admin." />
          <div className="mb-4 flex flex-wrap items-center gap-2">
            {reportStatusOptions.map((option) => (
              <button
                key={option.value}
            ...
            {reports.map((report) => (
              <div key={report.id} className="rounded-2xl border border-slate-200 p-4">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                  <div className="min-w-0">
                    <div className="text-sm font-semibold text-slate-900">{report.post?.content?.slice(0, 100) || "Bài viết bị báo cáo"}</div>
                    <div className="mt-1 text-xs text-slate-500">{report.reason || report.reportType || "No reason"} · {formatDate(report.createdAt)}</div>
                    <div className="mt-2 text-xs text-slate-500">Người báo cáo: {report.reporter?.fullName || report.reporter?.name || "Ẩn danh"}</div>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    <button onClick={() => updateReport(report.id, "Reviewed")} className="rounded-full bg-blue-50 px-3 py-1.5 text-xs font-semibold text-blue-700">Reviewed</button>
                    <button onClick={() => updateReport(report.id, "Resolved")} className="rounded-full bg-emerald-50 px-3 py-1.5 text-xs font-semibold text-emerald-700">Resolved</button>
                  </div>
                </div>
              </div>
            ))}
